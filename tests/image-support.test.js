...
test("member export answers include text-first visual aid metadata", async () => {
  const reply = [
  ...
  const directAids = visualAidsForAnswer(reply, "member email export");
  ...
  assert.equal(directAids[0].id, "member-data-export-guide");
