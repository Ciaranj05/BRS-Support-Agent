...
export function buildIntentFrame(message = "") {
  ...
  const negatesBuggyPrice = hasAny(lower, ["not asking about price", "not about price", "not the price", "not price", "not asking about rate", "not the rate", "not service rate"]);
  const buggyPriceIntent = hasAny(lower, BUGGY_TERMS) && hasAny(lower, BUGGY_PRICE_TERMS) && !negatesBuggyPrice;
