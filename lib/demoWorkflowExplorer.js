...
function resolveDemoBaseUrl(configuredBaseUrl = "", clubId = "") {
...
function getConfig() {
  const clubId = process.env.BRS_DEMO_CLUB_ID || process.env.BRS_CLUB_ID;
  return {
    baseUrl: resolveDemoBaseUrl(process.env.BRS_DEMO_BASE_URL || process.env.BRS_BASE_URL, clubId),
    clubId,
    ...
    browserExecutablePath: process.env.BRS_DEMO_BROWSER_EXECUTABLE_PATH || process.env.BRS_CRAWL_BROWSER_EXECUTABLE_PATH || "",
  };
}
...
async function gotoTimesheet(page, config) {
  const candidates = [
    `${config.baseUrl}/day.php`,
    `${config.baseUrl}/timesheet`,
    `${config.baseUrl}/tee_sheet.php`,
    `${config.baseUrl}/timesheet.php`,
  ];
...
async function exploreBookingSurfaces(page, config) {
  ...
  const commitResult = openedBooking && config.bookingCreationMode === "commit"
    ? await commitDemoBookingIfSafe(page)
    : { created: false, rollbackAttempted: false, rollbackVerified: false, evidence: null };
  const routes = [];
  ...
  routes.push({
    ...
    steps: [
      ...
      commitResult.created ? "Submit the temporary demo booking in the dedicated demo club." : "Keep the demo booking as a draft unless commit mode is explicitly enabled.",
      commitResult.rollbackVerified ? "Roll back the temporary demo booking and verify that the test record is no longer visible." : null,
    ].filter(Boolean),
    outcome: commitResult.created ? "A temporary test booking was created in the dedicated demo club." : "Booking-linked evidence is collected without submitting a booking.",
    verification: [
      "Return to the Timesheet day view and check the booking row evidence.",
      commitResult.created ? `Rollback verified: ${commitResult.rollbackVerified ? "yes" : "no"}.` : null,
    ].filter(Boolean),
  });
  ...
  return {
    routes,
    evidencePages: [timesheetEvidence, bookingEvidence, commitResult.evidence].filter(Boolean),
    writeActions: commitResult.created ? [{
      name: "Temporary demo booking",
      label: "Submit temporary booking",
      type: "safe-test-record-with-rollback",
      riskTier: "controlled-demo-write",
      rollbackPlan: "Delete, cancel, or otherwise remove the temporary test booking immediately after capturing evidence.",
      rollbackVerified: commitResult.rollbackVerified,
    }] : [],
  };
}
...
async function openBookingCandidate(page) {
  ...
  const candidate = candidates.find((item) => {
    ...
    if (/facebook|twitter|linkedin|instagram|mailto:|tel:|support/i.test(text)) return false;
    if (item.href) {
      try {
        const href = new URL(item.href, page.url());
        if (href.hostname !== current.hostname || !href.pathname.startsWith(current.pathname.split("/").slice(0, 2).join("/"))) return false;
      } catch {
        return false;
      }
    }
    return /available|book|booking|tee time|add|slot|time/i.test(text) && !/delete|cancel|remove|refund|payment|setup|configuration|facebook|twitter/i.test(text);
  });
...
async function commitDemoBookingIfSafe(page) {
  if (isBlockedMutationUrl(page.url())) return { created: false, rollbackAttempted: false, rollbackVerified: false, evidence: null };
  ...
  if (!await submit.count().catch(() => 0)) return { created: false, rollbackAttempted: false, rollbackVerified: false, evidence: null };
  await submit.click({ timeout: 5000 }).catch(() => null);
  await page.waitForLoadState("domcontentloaded", { timeout: 8000 }).catch(() => null);
  const evidence = await collectEvidence(page);
  const rollbackControl = page.locator("button, input[type='submit'], input[type='button'], a[role='button'], a").filter({
    hasText: /^(delete|remove|cancel booking|cancel|clear)$/i,
  }).first();
  const rollbackAttempted = Boolean(await rollbackControl.count().catch(() => 0));
  if (rollbackAttempted && !isBlockedMutationUrl(page.url())) {
    await rollbackControl.click({ timeout: 5000 }).catch(() => null);
    await page.waitForLoadState("domcontentloaded", { timeout: 8000 }).catch(() => null);
  }
  const postRollbackText = normalise(await page.locator("body").innerText({ timeout: 3000 }).catch(() => ""));
  const rollbackVerified = rollbackAttempted && !/brs demo workflow test|demo-workflow@example\.invalid/.test(postRollbackText);
  return { created: true, rollbackAttempted, rollbackVerified, evidence };
}

function buildWorkflowEntry({ question = "Explore demo booking workflows", routes = [], evidencePages = [], writeActions = [] } = {}) {
  ...
  return {
    ...
    rollbackVerified: writeActions.length ? writeActions.every((action) => action.rollbackVerified) : null,
    confidence: "needs-review",
    safeForChatbot: false,
    tags: [
      "demo-exploration",
      "multi-route-workflow",
      writeActions.length ? "controlled-write-tested" : "draft-or-readonly-exploration",
      writeActions.length && writeActions.every((action) => action.rollbackVerified) ? "rollback-verified" : null,
...
export async function exploreDemoWorkflows({ question = "Explore demo booking workflows" } = {}) {
  ...
  const browser = await chromium.launch({ headless: process.env.BRS_AUTOMATION_HEADLESS !== "false", executablePath: config.browserExecutablePath || undefined });
