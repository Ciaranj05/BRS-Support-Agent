import { normaliseImageAttachments } from "../visionContext.js";
...
export function validateChatInput(req, res, next) {
  ...
  try {
    req.body.attachments = normaliseImageAttachments(attachments || []);
  } catch (error) {
    return res.status(error.status || 400).json({ ok: false, error: error.message || "Invalid image attachment." });
  }
  ...
  function originAllowed(origin) {
    return allowedOrigins.some((allowed) => {
      ...
      if (allowed.startsWith("https://*.")) {
        ...
        return origin.endsWith(`.${suffix}`) && origin.startsWith("https://");
      }
      return false;
    });
  }
