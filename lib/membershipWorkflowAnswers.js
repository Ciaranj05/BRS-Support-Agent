...
export function isMemberBalanceReportQuestion(message = "") {
  ...
  if (hasAny(lower, ["competition", "competitions", "open competition", "competition purse", "entry fee"]) || /\bcomp\b/.test(lower)) return false;
  ...
  if (/\bwhat does [a-z][a-z'-]+\s+[a-z][a-z'-]+\s+owe\b/.test(lower) || /\b[a-z][a-z'-]+\s+[a-z][a-z'-]+'s\s+(?:member\s+)?balance\b/.test(lower)) return false;
  if (
