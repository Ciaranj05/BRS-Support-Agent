...
export function isMemberBalanceReportQuestion(message = "") {
  ...
  const asksForLookup = /\b(how|where|which|who|show|see|find|list|report|view|check|download|export)\b/.test(lower);
  ...
  return (asksForLookup || whatLookup) && hasMemberTerm(lower) && hasDebtConcept;
}
