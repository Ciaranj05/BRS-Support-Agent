...
const GUIDANCE_WORDS = /^(how|where|what)\b|\b(how do i|how to|where do i|what .*should .*check|what .*setup .*check|steps|guide|instructions)\b/i;
...
function isTimesheetContentGuidance(message) {
  return /\b(message|note|notice|warning|text|print|view|open|find|search|squeeze|interval|intervals|start of|end of|beginning of|first tee time|last tee time|earlier|later|more tee time|more tee times|extra tee time|extra tee times|additional tee time|additional tee times|booking|reservation|golfer|player|customer|visitor)\b/i.test(normaliseMessage(message));
}
