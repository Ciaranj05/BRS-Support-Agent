...
function isCheckInQuestion(lower = "") {
  return hasAny(lower, ["check in", "check-in", "checked in", "mark arrived", "mark as arrived", "arrived"]) &&
    hasAny(lower, ["player", "players", "golfer", "golfers", "member", "visitor"]);
}
...
function greenFeeChannel(lower = "") {
  if (!hasAny(lower, ["green fee", "green fees", "rate", "rates", "price", "prices", "charge", "charging", "cost"]) && !hasAny(lower, ["member guest", "member guests", "tour operator", "tee time agent"])) return "";
...
function buggyServicePriceAnswer() {
  return payload({
    ...
    reply: [
      ...
      "",
      "Use Tools > System Configuration only for the number of buggies available and online buggy booking settings. Tools > Services controls bookable extras and prices.",
    ].join("\n"),
  });
}
