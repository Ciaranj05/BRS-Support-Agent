...
function isStaffSelectedGreenFeeQuestion(lower = "") {
  if (shouldSkipGreenFeeDomainRoute(lower)) return false;
  ...
  return hasAny(lower, ["offline", "offline rate", "offline green fee", "manual", "manually", "manual booking", "office booking", "telephone booking", "email booking", "desk selects", "desk select", "rate the desk", "staff rate", "manual timesheet", "green fee dropdown", "phone booking", "pro shop dropdown"]) &&
...
function greenFeeChannel(lower = "") {
  ...
  const explicitStaffSignal = hasAny(lower, ["offline", "manual", "manually", "staff", "desk", "pro shop", "phone", "telephone", "email booking", "office booking", "rings", "dropdown", "select from", "timesheet"]);
  ...
  if (hasAny(lower, ["member guest", "member guests", "members guest", "members guests"])) return "member-guest-online";
  if (hasAny(lower, ["offline", "manual", "manually", "staff", "pro shop", "phone", "telephone", "email booking", "office booking", "rings", "dropdown", "select from", "timesheet"])) return "staff-manual";
...
function staffManualGreenFeeAnswer(channel = "staff-manual") {
  ...
  return payload({
    ...
    reply: [
      ...
      "",
      `1. Go to ${screen.path}.`,
      "2. Select the correct year for the period you are changing.",
      "3. Add a new rate with \"Add Green Fees\", or use the \"Actions\" column to edit an existing rate.",
      "4. Set the Category, Sub Category, Display Name, Course, Holes, Start Date, and End Date.",
      "5. Set the availability/rate rows for the days of the week. Enter the price for the days the rate is available, and use X or leave the day blank when the rate should not be available.",
      "6. For member/member-guest rates, check membership-plan applicability, Default, and Global before saving.",
      "7. Click \"Create\" or save the row, then test the matching staff Timesheet booking or member/member-guest online booking route.",
      "",
      "Use this for offline/manual staff bookings, such as telephone, email, office, pro-shop, and Timesheet dropdown rates. Do not use the visitor/agent online green-fee page for those staff-selected rates.",
      "",
      memberGuestNote,
    ].join("\n"),
  });
