...
const INTERNAL_OR_THIRD_PERSON_PATTERNS = [
  /\bdata\/export request\b/i,
  /\bemail-message request\b/i,
  /\bemail message request\b/i,
  /\bclub is asking\b/i,
  /\badvising staff\b/i,
  /\banother support agent\b/i,
  /\bBRS customers using\b/i,
];
...
export function hasInternalOrThirdPersonWorkflowWording(reply = "") {
  return INTERNAL_OR_THIRD_PERSON_PATTERNS.some((pattern) => pattern.test(String(reply || "")));
}

export function shouldBlockVagueWorkflowAnswer(message = "", reply = "") {
  if (!reply || (!hasVagueCustomerWorkflowAdvice(reply) && !hasInternalOrThirdPersonWorkflowWording(reply))) return false;
...
export function applyAnswerQualityGate(payload = {}, message = "") {
  ...
  if (payload.version === "domain-model-routing-v1") return payload;
  ...
  return {
    ...
    qualityGate: {
      blocked: true,
      reason: hasInternalOrThirdPersonWorkflowWording(payload.reply) ? "internal-or-third-person-workflow-wording" : "vague-or-unverified-workflow-advice",
