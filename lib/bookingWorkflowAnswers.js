...
export function isMoveBookingQuestion(message = "") {
  ...
  const bookingObject = /\b(booking|tee time|teetime|tee sheet|teesheet|timesheet|buggy|buggies|service|hire item|trolley|caddie|caddy|club hire|player|golfer|4-ball|4ball|fourball|four-ball|paid)\b/.test(lower);
...
8. Check the booking, services/extras, buggy or hire items, player count, and payment status after moving it.

Important:
Use Cut from inside Booking Details. If the booking has payment information attached, this is the route that keeps the payment information with the booking. Recheck any services or extras after the move because availability can depend on date, time, course, and service setup.`, relatedGuidesForQuestion(message || "move booking"));
}
