...
const INCOMPLETE_EVIDENCE_PATTERNS = [
  ...
  /\bconfirmed BRS page evidence\b/i,
  /\bUse the visible fields, filters, tabs, or actions shown on the page\b/i,
  /\bUse the visible field, filter, and selector labels on this page\b/i,
  /\bUse the visible action controls on this page as needed for the task\b/i,
  /\bCheck the visible table columns for the result or record details\b/i,
  /\bUse the confirmed fields\/filters on this screen\b/i,
  /\bUse the confirmed actions on this screen as needed\b/i,
  /\bCheck the result table columns\b/i,
...
export function hasIncompleteWorkflowEvidence(entry = {}) {
  ...
  const substantialUiEvidence = [
    entry.fields || [],
    entry.controls || [],
    entry.tableHeaders || [],
    entry.actions || [],
  ].some((items) => Array.isArray(items) && items.length >= 3);
  if (!substantialUiEvidence && LAUNCHER_ONLY_PATTERNS.some((pattern) => pattern.test(text))) return true;
