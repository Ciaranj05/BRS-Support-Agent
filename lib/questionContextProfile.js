...
function scenarioAnswerIssue(profile = {}, reply = "") {
  ...
  if (scenarios.prorataBilling && !includesAny(["Billing/Payments", "CREATE BILLS", "Billing Reference", "Due Date", "Who To Bill", "ADD ITEM", "PREVIEW", "Billing area"])) return "missing-prorata-billing-specifics";
  ...
  if (scenarios.flexibleMemberSync && !(includesAny(["Users"]) && includesAny(["Memberships", "Membership Type", "Flex", "Flexible", "Club Systems", "Member Category Mapping"]))) return "missing-flexible-member-specifics";
  ...
  if (scenarios.courseAvailability && !(includesAny(["course", "hole", "9 hole", "18 hole"]) && includesAny(["GolfNow", "Golf Now", "member booking", "visitor", "availability", "green fee"]))) return "missing-course-availability-specifics";
  return null;
}
...
function confidenceLine(profile = {}, confidence = null) {
  return `Confidence: ${confidence || profile.confidence || "medium"}.`;
}

function liveDataLine(profile = {}) {
  if (!profile.liveActionSignal && !hasAny(profile.lower || "", LIVE_DATA_AREAS)) return "";
...
function issueSpecificChecks(profile = {}, message = "") {
  ...
  if (scenarios.prorataBilling) {
    ...
    checks.push(`For the prorated member bill (${contextLead(profile)}), use "Memberships" rather than the Timesheet.${amountText}`);
    checks.push("For a single new member, open " + '"Memberships" > "Members"' + ", create or find the member profile, set the correct " + '"Membership Type"' + " and membership dates, then use that member's billing area for the one-off prorated charge.");
    checks.push("For the batch-billing route, open " + '"Memberships" > "Billing/Payments"' + ', click "CREATE BILLS", enter a clear "Billing Reference" and the correct "Due Date", then click "SET BILL FILTERS".');
    checks.push("Use the bill period/cycle option that matches a one-off prorated bill. If it is not part of a normal cycle, use the no-cycle/manual period route rather than pulling in the full annual cycle.");
    checks.push('In "Who To Bill", select only the relevant member. In the item/subscription step, use the correct subscription or "ADD ITEM" for the prorated line amount, then leave "Payment Schemes" blank unless the club wants instalments.');
    checks.push('Click "PREVIEW" and check the member, dates, reference, due date, line item, total, and publish/payment status before confirming.');
    return checks;
  }
  ...
  if (scenarios.courseAvailability) {
    ...
    checks.push("Check the course/hole setup for the affected course and date so BRS is not still treating it as an 18-hole product.");
    checks.push("Check member booking rules for the affected membership type, including release times, booking restrictions, and whether that member type can book the 9-hole course online.");
    checks.push("Check visitor/green-fee rates and online availability for the same date so visitor channels are not still offering 18-hole availability.");
    checks.push("Check GolfNow-facing setup separately for Hot Deals or visitor inventory because GolfNow display can differ from member booking rules.");
    checks.push("Use one affected member type, one visitor channel, one date, and one tee time as the test case before changing wider availability.");
    return checks;
  }
...
export function buildContextualRetrievalQueries(message = "", profile = buildQuestionContextProfile(message), intent = {}, evidencePlan = {}) {
  ...
  if (scenarios.courseAvailability || scenarios.greenFeeIssue) queries.push(
    'course hole setup 9 hole 18 hole GolfNow Hot Deals visitor online availability member booking rules green fee rates member online green fee rates',
  );
...
export function buildContextualSupportFallbackReply({ message = "", profile = buildQuestionContextProfile(message), reason = "contextual-synthesis-required", evidenceEntries = [] } = {}) {
  ...
  return [
    ...
    "",
    `${confidenceLine(profile)} The answer is based on the BRS area and workflow evidence available here; the exact live cause still depends on the club's records and settings.`,
    "",
