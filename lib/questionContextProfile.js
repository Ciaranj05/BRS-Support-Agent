...
function extractAreas(lower = "") {
  const areas = [];
  const golfNowAvailabilityContext = hasAny(lower, ["golfnow", "golf now"]) && hasAny(lower, [
    "availability",
    "available tee",
    "available times",
    "tee times",
  ...
  if (hasAny(lower, ["green fee", "rate", "rates", "visitor", "visitors", "9 hole", "9-hole", "18 hole", "18-hole"]) || golfNowAvailabilityContext) areas.push("visitor-online");
  if (hasAny(lower, ["report", "reports", "reporting", "export", "spreadsheet", "csv", "list"])) areas.push("reports");
...
function supportScenarioFlags(profile = {}) {
  ...
  const reportWords = hasAny(lower, ["report", "reports", "reporting", "reporting functions", "export", "download", "spreadsheet", "csv", "showing how many", "specified period", "list"]);
  const messagingWords = hasAny(lower, ["message", "messages", "text", "sms", "club message", "not receiving", "not received"]) || (
    hasAny(lower, ["email", "emails"]) &&
    hasAny(lower, ["send", "sending", "sent", "receive", "received", "not receiving", "not received", "newsletter", "mailshot", "address", "addresses", "template"])
  );
  const golfNowAvailabilityWords = hasAny(lower, ["golfnow", "golf now"]) && hasAny(lower, [
    "availability",
    "available tee",
    "available times",
    "tee times",
  ...
  return {
    ...
    courseAvailability: hasAny(lower, ["9 hole", "9-hole", "18 hole", "18-hole", "hot deals", "cannot book", "can't book", "cant book", "online availability", "member booking rules"]) || golfNowAvailabilityWords,
    ...
    greenFeeIssue: hasAny(lower, ["green fee", "green fees", "visitor rate", "visitor rates", "member rate", "rates"]),
  };
}
...
function scenarioAnswerIssue(profile = {}, reply = "") {
  ...
  const hasSpecificNonReportingScenario = Boolean(
    scenarios.prorataBilling ||
    scenarios.contactsMissing ||
    scenarios.contactsReAdd ||
    scenarios.guestReport ||
    scenarios.competitionPurse ||
    scenarios.competitionReconciliation ||
    scenarios.flexibleMemberSync ||
    scenarios.bookingPaymentPayout ||
    scenarios.courseAvailability ||
    scenarios.greenFeeIssue ||
    scenarios.messagingIssue ||
    scenarios.usersIssue ||
    scenarios.facilitiesIssue ||
    scenarios.timesheetIssue
  );
  ...
  if (scenarios.reportingLookup && !hasSpecificNonReportingScenario && !(
    includesAny(["Reports"])
    && includesAny(["Type of Report", "Start Date", "End Date", "Print Report", "export", "download"])
    && includesAny(["booking reports", "payment reports", "contact reports", "Memberships reports", "member bills", "wallet data"])
  )) return "missing-reporting-lookup-specifics";
  return null;
}
...
export function buildQuestionContextProfile(message = "") {
  ...
  const reportSignal = hasAny(lower, ["report", "reports", "reporting", "reporting functions", "showing how many", "export", "download", "spreadsheet", "list", "reconcile", "reconciliation"]);
  const broadReportingHelpSignal = hasAny(lower, ["reporting functions", "use reports better", "utilise the reporting", "utilize the reporting", "help with reports", "help me with reports", "reporting help"]);
  const liveActionSignal = hasAny(lower, ["can you look", "take a look", "look into", "fix this", "sort this", "do this for us", "can you confirm", "confirm if", "can you check", "check this for us"]);
  ...
  const contextualReportCase = reportSignal && Boolean(broadReportingHelpSignal || contextualReportingCase || problems.length || (narrativeSignal && (areas.length > 1 || facts.length > 0)));
...
export function contextualAnswerIssue(message = "", reply = "") {
  ...
  if (/Confidence:\s*(?:high|medium|low)/i.test(reply)) return "visible-confidence";
  if (!profile.liveActionSignal && /I cannot inspect or change the club's live BRS records|I cannot inspect your live club|I cannot inspect the club's live BRS data/i.test(reply)) return "unneeded-live-data-disclaimer";
...
function escalationLine(profile = {}) {
  const scenarios = supportScenarioFlags(profile);
  if (scenarios.contactsMissing || scenarios.contactsReAdd) {
    return "Escalate if the contact still cannot be found or restored after checking search, filters, category/site, and the re-add route.";
  }
  if (scenarios.flexibleMemberSync) {
    return "Escalate before creating a duplicate member profile or changing sync/mapping if the linked user/member relationship still does not explain it.";
  }
  if (scenarios.courseAvailability) {
    return "Escalate before changing wider live availability if one member type, one visitor channel, one date, and one tee time do not explain the issue.";
  }
  if (scenarios.competitionPurse || scenarios.competitionReconciliation || scenarios.bookingPaymentPayout) {
    return "Escalate before changing payments, refunds, or competition charges if the checks do not explain the issue.";
  }
  if (scenarios.usersIssue) {
    return "Escalate before changing permissions broadly if the user account, role/group, and linked profile checks do not explain the issue.";
  }
  return "Escalate if the checks do not resolve it.";
}
...
export function buildContextualSupportFallbackReply({ message = "", profile = buildQuestionContextProfile(message), reason = "contextual-synthesis-required", evidenceEntries = [] } = {}) {
  ...
  const checksIncludeEscalation = checks.some((check) => /\bescalate\b/i.test(check));
  const title = (() => {
    const scenarios = supportScenarioFlags(profile);
  ...
  return [
    ...
    checksIncludeEscalation ? null : escalationLine(profile),
