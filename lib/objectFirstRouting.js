...
function competitionReply(lower) {
  ...
  if (wantsChargeOrPayment(lower) && hasMemberAudience(lower)) {
    return response(`For member competition charges, use the competition purse/charges route rather than membership billing or a generic payment request.

Check the competition setup, charges, and purse/payment settings for the competition, then confirm the member entry fee is being taken from the member's competition purse.

If visitors are also entering, handle visitor/open competition fees separately from member purse charging.`, "teesheet", [], "guardrail");
  }
...
Look for the billing report for unpaid/outstanding member bills, then filter or export the report as needed.`, "memberships", [], "generic");
  }
