...
function isSocietyBlockBookingIntent(lower = "") {
  ...
  const blockSignal = hasAny(lower, ["consecutive", "fourball", "fourballs", "4-ball", "4balls", "blocked off", "block off", "block each", "multiple tee times", "8 or 9", "names later", "visitors grabbing"]) ||
    ...
    (hasAny(lower, ["reserve", "reserved", "hold", "held"]) && hasAny(lower, ["slots", "tee slots", "tee times", "time range", "names later", "visitors grabbing", "multiple", "online"]));
...
function isMemberBalanceLiveDataRequest(lower = "") {
  return hasAny(lower, ["show me", "list", "give me", "who owes", "who still owes", "who hasn't paid", "who hasnt paid", "all unpaid", "all outstanding"]) &&
...
function competitionScoringAnswer() {
  return answer("Check Competition Scoring or Leaderboard Integrations", [
    "First separate the BRS competition setup from the scoring system. BRS can manage competition setup, entries, draws/start sheets, charges, and online entry, but scoring and leaderboards may depend on an enabled scoring integration.",
    ...
    "If scores or the leaderboard are wrong inside the scoring product, the club will usually need to contact that scoring provider.",
