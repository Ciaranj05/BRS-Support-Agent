...
export function approvedStaticWorkflowReply(message = "") {
  ...
  if (hasAny(lower, ["cancel", "delete", "remove"]) && hasAny(lower, ["booking", "reservation"]) && (mentionsTimesheet || hasAny(lower, ["tee time", "tee sheet"])) && !hasAny(lower, ["tee time slot", "tee time pattern", "all tee times", "empty tee time"])) {
    return detailedAnswer(
