...
export function approvedStaticWorkflowReply(message = "") {
  ...
  if (hasAny(lower, ["set tee times", "change time intervals", "time intervals", "configure timesheet", "configure tee sheet", "configure tee shet", "conifgure tee shet", "setup tee sheet", "set up tee sheet"]) || (hasAny(lower, ["next year", "intervals", "tee times"]) && hasAny(lower, ["timesheet", "tee sheet", "teesheet", "tee shet", "shet"]))) {
  ...
  if ((lower.includes("brs payments") && lower.includes("refund")) || (hasAny(lower, ["online payment", "brs payment"]) && lower.includes("refund") && lower.includes("list")) || (lower.includes("refund") && hasAny(lower, ["record", "records", "history", "after a refund", "see refund", "view refund"]))) {
