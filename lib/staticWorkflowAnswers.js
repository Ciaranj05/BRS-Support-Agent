...
function isBookingPaymentRefundIntent(lower = "") {
  const refundSignal = hasAny(lower, ["refund", "money back", "return money", "return card", "return green fee", "green fee back", "reverse", "refund button", "where is the refund button"]);
  const bookingPayment = hasAny(lower, ["booking", "tee booking", "tee time", "saturday golf", "visitor", "visitor green fee", "green fee paid", "paid online", "online payment", "brs payments", "card payment"]);
...
function isBRSPayoutsIntent(lower = "") {
  return hasAny(lower, ["payout", "payouts", "paid us out", "bank deposit", "deposit from brs"]) &&
    hasAny(lower, ["report", "summary", "review", "find", "last week", "match", "does not match", "transactions", "bank", "online payments", "brs"]);
}
