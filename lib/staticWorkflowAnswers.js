...
export function approvedStaticWorkflowReply(message = "") {
  ...
  if (hasAny(lower, ["change", "update", "edit", "modify", "amend", "correct"]) && hasAny(lower, ["member", "members"]) && hasAny(lower, ["email", "email address", "phone", "telephone", "mobile", "address", "name", "surname", "first name", "last name", "details", "contact details"])) {
    ...
    ], "Check: If the member also has a login account, check Users to confirm their login email matches if needed.");
  }
      ...
      [
        {
          ...
          items: [
            ...
            "Update the Calendar End Year in System Configuration if needed for tee time access.",
          ],
        },
      ]
    );
  ...
  if (hasAny(lower, ["set up", "setup", "create", "add", "configure"]) && hasAny(lower, ["facility", "room", "resource"]) && !hasAny(lower, ["booking", "reservation", "book"])) {
    return answer("Set Up a New Facility or Room", [
      ...
      "Look for the facility/room setup or configuration area.",
      "Add a new facility or room with the required name, capacity, and availability settings.",
      "Set the booking rules (minimum/maximum duration, available times, days of week).",
      "Save the new facility and check it appears in the Facilities booking grid.",
    ], "Note: Facility setup options vary by club configuration. If the setup option is not visible, check your user permissions or contact BRS Support for assistance.");
  }
  ...
  if (hasAny(lower, ["cancel", "delete", "remove"]) && hasAny(lower, ["facility booking", "room booking", "facility reservation", "room reservation"]) || (hasAny(lower, ["cancel", "delete"]) && hasAny(lower, ["facility", "room"]) && hasAny(lower, ["booking", "reservation"]))) {
    return answer("Cancel a Facility Booking", [
      ...
      "Use the Delete or Cancel option to remove the reservation.",
  ...
  if (hasAny(lower, ["rate", "rates", "price", "prices", "pricing", "charge", "cost"]) && hasAny(lower, ["facility", "room", "resource"])) {
    return answer("Set Facility Booking Rates", [
      "Open Facilities from the main navigation menu.",
      "Look for the Rates or Pricing configuration within Facilities.",
      "Set the rate per hour, per session, or per booking as required by the club.",
      "Configure different rates for different room types or time periods if needed.",
      "Save the rates and check they appear correctly when making a new facility booking.",
    ], "Note: Facility rate configuration depends on the club's Facilities module setup. If rate options are not visible, check Tools > System Configuration or contact BRS Support.");
  }
  ...
  if (hasAny(lower, ["sent message", "sent messages", "message history", "messages i sent", "messages we sent", "find sent", "view sent", "previous message", "previous messages", "old message", "old messages"]) && !hasAny(lower, ["text", "txt", "sms", "club message"])) {
    return answer("View Sent Messages", [
      "For recently sent emails: Go to Tools > Email Messaging. Check the sent or recent email history if available.",
      "For recently sent text messages: Go to Tools > Text Messaging and look for sent text history or SMS reports.",
      "For recently sent club messages: Open Messages from the main navigation menu and check the club message history.",
    ], "Note: The exact location depends on whether you sent an email, text, or club message. Check the relevant messaging area for history.");
  }
