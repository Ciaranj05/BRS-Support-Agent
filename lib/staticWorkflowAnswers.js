...
function isContactReportExportIntent(lower = "") {
  ...
  return hasContactRecordSubject(lower) &&
    ...
    !hasAny(lower, ["send", "sending", "mailshot", "newsletter", "email all"]);
}
...
function isContactCategorySetupIntent(lower = "") {
  return hasAny(lower, ["contact category", "contact categories", "contact cat", "contact cats", "contact type", "contact types", "contct category"]) &&
...
function isAmbiguousContactPrompt(lower = "") {
  return ["contact", "contacts", "visitor details"].includes(lower);
}
