...
function isMembershipBillPaymentReconciliationIntent(lower = "") {
  ...
  const paymentSignal = hasAny(lower, [
    "paid",
    "payment",
    "payments",
    "record a payment",
...
function isMemberBalanceLiveDataRequest(lower = "") {
  ...
  return asksForLiveList && (memberMoneySignal || personalOutputSignal || subsLiveListSignal);
}
...
function isMemberLoginAccessIntent(lower = "") {
  ...
  const setupSignal = hasAny(lower, ["new", "create", "add", "set up", "setup", "where", "do i add", "do i enable", "can't", "cant", "cannot", "joined today", "joining today", "after joining", "disabled", "forgot"]);
