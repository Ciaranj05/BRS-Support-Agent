...
export function approvedStaticWorkflowReply(message = "") {
  ...
  if (hasAny(lower, ["booking rules", "booking restrictions", "advance booking", "player limits", "booking window", "who can book", "online availability rule"])) {
    return answer("Check Booking Rules", [
      "Clarify who the rule applies to: members, visitors, open competitions, or a course/group-size restriction.",
  ...
  if (
    ...
    ], "Check: Use Users only when the request is about a staff/admin login account rather than a member registration or member profile.");
  }
