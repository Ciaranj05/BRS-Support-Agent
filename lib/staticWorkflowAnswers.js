...
function isLiveMutationInstruction(lower = "") {
  ...
  const liveObject = hasAny(lower, ["booking", "tee time", "tee slot", "member", "members", "customer", "visitor", "visitors", "database", "email", "message", "payment", "balance", "competition", "comp", "entry", "entrant", "draw", "start sheet", "captain's prize", "captain prize", "medal", "scratch cup", "green fee", "rate table", "rates", "price", "tour operator", "member guest"]);
...
function isCompetitionPaymentIntent(lower = "") {
  return hasAny(lower, ["competition", "open competition", "open comp", "entry fee", "competition purse", "purse"]) &&
    hasAny(lower, ["payment", "paid", "paying", "fee", "fees", "refund", "withdrawal", "withdrew", "green fee", "charge", "charges"]);
}
...
function isCompetitionReportsWebsiteIntent(lower = "") {
  if (isCompetitionScoringIntent(lower)) return false;
  ...
  return hasCompetitionTerm(lower) && (reportSignal || (websiteSignal && hasAny(lower, ["result", "results", "report", "reports"])));
}
...
function isTimesheetBookingCreationIntent(lower = "") {
  ...
  const buggyTerm = hasAny(lower, ["buggy", "buggies", "golf cart", "golf carts"]);
  const bookingAvailabilityQuestion = buggyTerm &&
    hasAny(lower, ["visitor", "visitors", "member", "members", "book online", "online booking", "book buggy", "book buggies", "book golf cart", "book golf carts"]) &&
    !hasAny(lower, ["add a buggy service", "add buggy service", "add a buggy", "add buggy", "service to a booking", "services to a booking", "existing booking", "tee booking", "booking details", "hire price", "service rate"]);
  if (bookingAvailabilityQuestion) return false;
...
export function approvedStaticWorkflowReply(message = "") {
  ...
  if (lower.includes("copy") && hasAny(lower, ["services", "green fee", "green fees", "green fee rates", "catering", "prices", "buggies", "buggy"]) && hasAny(lower, ["year", "next year", "another year", "from year", "to year", "2026", "2027"])) {
  ...
  const staffManualGreenFeeSetup = !hasAny(lower, ["report", "reports", "count", "counts", "online versus offline", "online vs offline"]) &&
    hasAny(lower, ["offline", "manual", "manually", "staff", "pro shop", "phone", "telephone", "email booking", "office booking", "timesheet dropdown", "green fee dropdown"]) &&
    hasAny(lower, ["green fee", "green fees", "rate", "rates", "price", "prices"]) &&
    hasAny(lower, ["setup", "set up", "configure", "add", "create", "change", "edit", "where", "missing", "dropdown"]);

  if (staffManualGreenFeeSetup) {
    return answer("Set Staff-Selected Green Fee Rates", [
      "Go to Tools.",
      "Open Green Fee Rates.",
      "Select the correct year for the period you are changing.",
      "Click Add Green Fees to add a new offline/manual staff rate, or use the Actions column to edit an existing rate.",
      "Set Category, Sub Category, Display Name, Course, Holes, Start Date, and End Date.",
      "Set the availability/rate rows for each day of the week. Enter the price where the rate is available, and use X or leave the day blank where it should not be available.",
      "For member/member-guest rates, check membership-plan applicability, Default, and Global before saving.",
      "Click Create or save the row, then test the matching staff Timesheet booking or member/member-guest online booking route.",
    ], "Use this for offline/manual staff bookings, such as telephone, email, office, pro-shop, and Timesheet dropdown rates. Do not use Green Fee Rates for Visitors / Agents for staff-selected rates.");
  }
  ...
  if ((lower.includes("day ticket") || (hasAny(lower, ["two rounds", "2 rounds", "same day"]) && lower.includes("visitor"))) && hasAny(lower, ["rate", "rates", "price", "prices", "round", "rounds", "book", "online"])) {
  ...
  if (lower.includes("copy") && hasAny(lower, ["services", "green fee", "green fees", "green fee rates", "catering", "prices", "buggies", "buggy"])) {
    return answer("Copy Services, Catering, or Green Fees to Another Year", [
      "Go to Tools.",
      "Open Copy Services, Catering or Green Fees.",
      "In Operation, choose Copy Services, Copy Catering, or Copy Green Fees.",
