...
function isLiveMutationInstruction(lower = "") {
  ...
  const liveObject = hasAny(lower, ["booking", "tee time", "tee slot", "member", "members", "customer", "visitor", "visitors", "database", "email", "message", "payment", "balance"]);
...
function isSocietyBlockBookingIntent(lower = "") {
  ...
  const groupSignal = hasAny(lower, ["society", "societies", "charity day", "corporate day", "golf day", "rugby club", "organiser", "organizer", "shotgun-ish", "shotgun"]);
...
function isPartialBookingRefundIntent(lower = "") {
  ...
  return refundAction && bookingPayment && partialSignal;
}
...
function isMemberBalanceLiveDataRequest(lower = "") {
  const asksForLiveList = hasAny(lower, ["show me", "list", "give me", "who owes", "who still owes", "who hasn't paid", "who hasnt paid", "all unpaid", "all outstanding"]);
  ...
  return asksForLiveList && (memberMoneySignal || subsLiveListSignal);
}
...
function isMemberBookingPrivilegeRuleIntent(lower = "") {
  ...
  const categorySignal = hasAny(lower, ["5-day", "five-day", "five day", "7-day", "seven-day", "seven day", "wrong category", "membership type", "membership category", "member category"]);
  const accessSignal = hasAny(lower, ["can book", "book saturday", "book weekend", "weekend", "saturday", "sundays", "category", "privilege", "privileges", "rules", "member app", "members app", "booking app", "tee times", "online"]) || /\bapp\b/.test(lower);
  const lookupSignal = hasAny(lower, ["where", "check", "why", "should i", "should we", "able to", "not able", "what controls", "controls that", "control", "controls", "getting"]);
...
function competitionScoringAnswer() {
  return answer("Check Competition Scoring or Leaderboard Integrations", [
    ...
    "Escalate to BRS Support when the BRS competition, entrants, draw/start sheet, or integration data is not syncing as expected.",
...
function isMemberLoginAccessIntent(lower = "") {
  ...
  const setupSignal = hasAny(lower, ["new", "create", "add", "set up", "setup", "where", "do i add", "do i enable", "can't", "cant", "cannot", "joined today"]);
...
function memberLoginAccessAnswer() {
  return answer("Check Member Login and Registration Access", [
    ...
    "In Memberships, check the member profile, membership type/category, membership status, and registration/enablement details.",
  ...
  ], "If the club cannot find the Member Booking link, registration record, or linked Users account, escalate to BRS Support with the member name, username/email, and screenshots of both Memberships and Users.");
}
...
function isMissingTimesheetBookingIntent(lower = "") {
  ...
  const bookingSignal = hasAny(lower, ["booking", "booked", "tee time", "tee slot", "confirmation", "visitor", "customer", "golfer", "member"]);
...
function staffChangedBookingDisappearedAnswer() {
  return answer("Check a Booking After Staff Changed It", [
    ...
    "If there are duplicate bookings, decide which one is correct before deleting the duplicate.",
...
function isMemberAppPaymentVisibilityQuestion(lower = "") {
  return hasAny(lower, ["member", "app"]) &&
    hasAny(lower, ["payment", "paid", "can't see payment", "cant see payment", "payment missing"]) &&
...
function visitorBookingConfirmationIssueAnswer() {
  return answer("Check a Visitor Online Booking Confirmation Issue", [
    ...
    "Escalate to BRS Support with the reference, email, date, course, and payment status if the confirmation and Timesheet/Search records do not match.",
...
function bulkBookingGuardrailAnswer() {
  return answer("Bulk Booking Change Guardrail", [
    ...
    "Escalate to BRS Support or the club manager if the change affects many bookings or needs a reversible bulk process.",
...
function isBlockedVisitorAvailabilityIntent(lower = "") {
  const visitorSignal = hasAny(lower, ["visitor", "visitors", "public", "online", "book online", "book a time"]);
  const blockedSignal = hasAny(lower, ["blocked", "block", "reserved", "restriction", "course restriction", "closed", "still book", "still available", "showing online"]);
  const verifySignal = hasAny(lower, ["verify", "check", "why", "thought", "shouldn't", "shouldnt", "not supposed"]);
...
export function approvedStaticWorkflowReply(message = "") {
  ...
  if (isCompetitionScoringIntent(lower)) {
    return competitionScoringAnswer();
  }
  ...
  if (
    ...
    (
      hasAny(lower, ["unpaid", "not paid", "non-payer", "non payer", "subscription", "subs", "renewal bill"]) &&
      hasAny(lower, ["lose app access", "stop online booking", "stop booking", "restrict booking", "booking access", "app access", "auto stop"])
    )
  ) {
