...
export function approvedStaticWorkflowReply(message = "") {
  ...
  if (lower.includes("grace period")) {
    return answer("Clarify the Grace Period Area", [
      "Check which BRS area the grace period relates to before giving steps.",
      "If it is about a member or subscription payment, handle it from the Memberships/member billing area.",
      "If it is about online booking access or how far ahead members can book, check Tools > Member Booking Rules.",
      "If it is about a competition, visitor booking, or another payment route, use that specific setup area instead.",
    ], "I do not have one proven generic BRS workflow called Grace Period, so the safe next step is to identify the exact area first rather than guess.");
  }
  ...
  if (hasAny(lower, ["club email address", "club email", "club e-mail", "club email contact", "club contact email"])) {
    return answer("Change the Club Email Address", [
      ...
      "Find the club address and email settings.",
      "Edit the club email address field.",
      "Save the change, then check a booking confirmation or club email template if the club needs to confirm which address customers will see.",
    ], "Check: If the club is asking about the wording of confirmation emails rather than the sender/contact email address, use Tools > Email and Letter Templates instead.");
  }
  ...
  if (hasAny(lower, ["main club setup", "club setup page", "system configuration", "feature on or off", "turn a feature", "enable feature", "disable feature"])) {
    return answer("Open System Configuration", [
      "Go to Tools.",
      "Open System Configuration.",
      "Use this area for system parameters, including the club address, club email, and enabled or disabled features.",
      "Edit the relevant setting, save the change, then check the affected BRS area before advising staff it is complete.",
    ]);
  }
  ...
  if (hasAny(lower, ["cutoff window", "cut off window", "cut-off window", "cancellation time limit", "cancel time limit", "deadline for members to book", "advance booking days", "how far ahead members", "how far in advance members"])) {
    return answer("Check Member Booking Rules", [
      ...
      "Review the booking window, cancellation, and deadline settings available for the relevant member group or rule.",
      "Save only after confirming the club wants the rule changed.",
  ...
  if (hasAny(lower, ["set tee times", "change time intervals", "time intervals", "configure timesheet", "configure tee sheet", "setup tee sheet", "set up tee sheet"]) || (hasAny(lower, ["next year", "intervals", "tee times"]) && hasAny(lower, ["timesheet", "tee sheet", "teesheet"]))) {
    return answer("Configure the Timesheet", [
      "Go to Tools.",
      "Open Configure Timesheet.",
      "Choose the year, date range, course, first tee time, last tee time, and interval that the club needs.",
      "Run the configure action, then open the Timesheet to check the generated tee times.",
    ], "Check: Timesheet configuration can affect many slots, so confirm the date range and course before saving.");
  }
  ...
  if (hasAny(lower, ["upload a tee sheet", "upload tee sheet", "upload timesheet", "upload a timesheet", "paste a list of fourballs", "paste list of fourballs"]) || (hasAny(lower, ["upload", "paste"]) && hasAny(lower, ["timesheet", "tee sheet", "teesheet", "fourballs"]))) {
    ...
    ], "Check: Upload Timesheet does not automatically email members, so use the normal messaging route separately if the club needs to notify people.");
  }
  ...
  if ((hasAny(lower, ["message", "notice", "text"]) && hasAny(lower, ["top of the tee sheet", "top of tee sheet", "top of the timesheet", "top of timesheet", "message at top"])) || (hasAny(lower, ["messages on the timesheet", "timesheet messages"]) && hasAny(lower, ["change", "set", "edit", "add"]))) {
    return answer("Set Messages on the Timesheet", [
      ...
      "Choose the date, course, or timesheet area where the message should appear.",
      "Enter or update the message text.",
      "Save the message and return to the Timesheet to confirm it appears in the correct place.",
    ]);
  }
  ...
  if (!lower.includes("copy") && !hasAny(lower, ["competition", "comp"]) && hasAny(lower, ["green fee rates", "green fees", "visitor prices", "visitor price"])) {
  ...
  if (hasAny(lower, ["buggy", "caddie", "caddy", "trolley", "club hire", "room hire"]) && hasAny(lower, ["services", "service", "bookable", "hire", "setup", "set up"])) {
  ...
  if (lower.includes("email template") || lower.includes("letter template") || lower.includes("confirmation email template") || (hasAny(lower, ["template", "templates"]) && hasAny(lower, ["email", "letter", "confirmation"]))) {
    return answer("Set Up Email and Letter Templates", [
      ...
      "Create or edit the template the club needs, such as a confirmation email or letter template.",
      "Check the template content and where it is used before saving it for live use.",
    ]);
  }
