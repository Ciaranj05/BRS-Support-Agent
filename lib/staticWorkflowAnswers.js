...
export function approvedStaticWorkflowReply(message = "") {
  ...
  if (hasAny(lower, ["set tee times", "change time intervals", "time intervals", "configure timesheet", "configure the timesheet", "configure tee sheet", "configure the tee sheet", "configure tee shet", "configure the tee shet", "conifgure tee shet", "conifgure the tee shet", "setup tee sheet", "set up tee sheet"]) || (hasAny(lower, ["next year", "next month", "intervals", "tee times"]) && hasAny(lower, ["configure", "conifgure", "set up", "setup", "change"]) && hasAny(lower, ["timesheet", "tee sheet", "teesheet", "tee shet", "shet"]))) {
  ...
  if (hasAny(lower, ["membership type", "membership types", "member type", "member types"]) && hasAny(lower, ["senior", "junior", "category", "categories", "set up", "setup", "create", "change", "edit"])) {
  ...
  if (hasAny(lower, ["payment scheme", "payment schemes", "payment plan", "payment plans", "instalment", "instalments", "installment", "installments", "scheduled payment", "scheduled payments"]) && hasAny(lower, ["create", "add", "new", "set up", "setup", "manage", "change", "edit", "scheduled"])) {
    return answer("Set Up a Membership Payment Scheme", [
      "Open Memberships.",
