...
function normaliseSourceType(sourceType = "manual") {
  return sourceType === "brs-system" ? "system" : sourceType;
}

function flattenLabels(items = []) {
  return items.map((item) => {
    if (typeof item === "string") return item;
    return [item.label, item.helpText, item.type].filter(Boolean).join(": ");
  });
}
...
export function normaliseKnowledgeEntry(entry = {}) {
  ...
  return {
    ...
    sourceUrl: sourceType === "system" ? null : entry.sourceUrl || null,
