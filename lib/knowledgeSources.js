...
const QUERY_TOKEN_EXPANSIONS = [
  {
    when: ["cancel", "cancelled", "canceled", "cancelling", "canceling"],
    add: ["delete", "deleted", "remove", "removed", "clear", "cleared", "cancellation"],
  },
  {
    when: ["delete", "deleted", "remove", "removed"],
    add: ["cancel", "cancelled", "canceled", "clear", "cleared"],
  },
  {
    when: ["booking", "bookings", "booked"],
    add: ["reservation", "reservations", "timesheet", "tee", "time"],
  },
  {
    when: ["reservation", "reservations"],
    add: ["booking", "bookings", "timesheet", "tee", "time"],
  },
  {
    when: ["edit", "change", "update", "amend"],
    add: ["modify", "modified", "manage"],
  },
  {
    when: ["modify", "modified"],
    add: ["edit", "change", "update", "amend"],
  },
  {
    when: ["find", "lookup", "look"],
    add: ["search", "retrieve", "filter"],
  },
  {
    when: ["download", "export", "spreadsheet", "csv"],
    add: ["report", "reports", "file", "list"],
  },
];
...
function expandQueryTokens(query = "") {
  ...
  for (const rule of QUERY_TOKEN_EXPANSIONS) {
    if (!rule.when.some((token) => tokenSet.has(token))) continue;
    expanded.push(...rule.add);
  }
  return expanded;
}
...
export function normaliseKnowledgeEntry(entry = {}) {
  ...
  return {
    ...
    reviewPayloadWithheld: Boolean(entry.reviewPayloadWithheld),
  };
}
