...
function normaliseCanonicalTitle(value = "") {
  ...
  return title
    ...
    .replace(/^text messages$/g, "send a text")
    .replace(/^member redacted name for email and text$/g, "membership groups for email and text")
    .replace(/^demo explored routes$/g, "timesheet")
    .replace(/^terms and conditions all ireland open competitions search facility$/g, "open competitions")
...
function isReviewedNonActionableSource(entry = {}, titleKey = "") {
  return isReviewPlaceholder(entry) &&
    ["sensitive-or-live-crawl-data", "requires-human-review"].includes(entry.reviewReason) &&
    ["0", "0%", "untitled knowledge entry"].includes(titleKey);
}

function canRetireByCoverage(entry = {}) {
  return ["incomplete-workflow-evidence", "sensitive-or-live-crawl-data"].includes(entry.reviewReason);
}
...
export function resolveReviewQueue(entries = []) {
  ...
  for (const entry of reviewEntries) {
    ...
    if (isReviewedNonActionableSource(entry, titleKey)) {
      retired.push({
        id: entry.id,
        title: entry.title,
        ...
        resolution: "reviewed-non-actionable-empty-or-page-chrome-source",
        approvedMatch: null,
      });
      continue;
    }
    ...
    const canRetire = canRetireByCoverage(entry) &&
    ...
    retired.push({
      ...
      reviewReason: entry.reviewReason,
      driverFamily: assignment.driverFamily,
      resolution: entry.reviewReason === "sensitive-or-live-crawl-data"
        ? "sensitive-source-superseded-by-approved-same-family-knowledge"
        : "superseded-by-approved-same-family-knowledge",
