...
export async function saveLearnedWorkflowFromLiveAnswer({ question = "", answer = "", liveResult = null, intent = {}, topic = null } = {}) {
  if (!question || !answer || !liveResult?.successful) return null;
  const inferredIntent = intent?.topic ? intent : inferIntentFromHistory([
  ...
  const familyEntry = buildWorkflowFamilyEntry({
    ...
    intent: inferredIntent,
    routes: entry.routes || [],
    evidencePages: liveResult.pages || [],
    source: "live-safety-net-verified-answer",
    explorationStatus: "observed-live-and-auto-learned",
  });
  const learnedWorkflow = await saveLearnedWorkflow({
    ...entry,
    confidence: "approved",
    safeForChatbot: true,
    tags: [...new Set([...(entry.tags || []), "live-safety-net-verified-answer", "auto-learned-from-live-lookup"])],
  });
  const learnedFamily = await saveLearnedWorkflow({
    ...familyEntry,
    tags: [...new Set([...(familyEntry.tags || []), "live-safety-net-verified-answer", "auto-learned-from-live-lookup"])],
  });
  return { ...learnedWorkflow, workflowFamily: learnedFamily };
}
