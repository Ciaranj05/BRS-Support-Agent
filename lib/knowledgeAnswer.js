...
import { buildContextualRetrievalQueries, buildContextualSupportFallbackReply, buildQuestionContextProfile, contextualAnswerIssue } from "./questionContextProfile.js";
import { withTimeout } from "./openaiHelpers.js";
import { formatLiveEvidence } from "./liveBrsLookup.js";
import { buildDeveloperAnswerTrace } from "./answerTrace.js";
...
function evidenceDedupeKey(entry = {}) {
  return [
    ...
    String(entry.content || "").replace(/\s+/g, " ").trim().slice(0, 500),
  ].join("|").toLowerCase();
}
...
function exactPhraseBonus(query = "", text = "") {
  ...
  const phrases = [
    ...
    "minimum member competition purse balance",
    "competition purse transactions",
    "process competition charges",
    "brs payments",
    "transactions",
    "payouts",
    "revenue from online merchant payments",
    "payments by payment date",
    "payments by transaction date",
    "member category mapping",
    "flex members",
  ];
  return phrases.reduce((score, phrase) => {
    if (!lowerQuery.includes(phrase) || !lowerText.includes(phrase)) return score;
    return score + 9;
  }, 0);
}

function matchedTokenDensityBonus(query = "", text = "", entry = {}) {
  const queryTokens = unique(tokenize(query).filter((token) => token.length > 2), 80);
  if (!queryTokens.length) return 0;
  const haystack = new Set(tokenize(text));
  const matched = queryTokens.filter((token) => haystack.has(token)).length;
  const density = matched / Math.min(queryTokens.length, 30);
  let bonus = Math.min(10, Math.round((matched * 0.65) + (density * 6)));
  if (entry.sourceType === "manual" && matched >= 4) bonus += 6;
  if (/case evidence/i.test(`${entry.title || ""} ${entry.id || ""}`) && matched >= 3) bonus += 3;
  return bonus;
}

function objectMismatchPenalty(query = "", entry = {}) {
  const lowerQuery = normalise(query);
  const titlePath = normalise([entry.title, entry.area, entry.navigationPath, entry.workflowFamily].join(" "));
  const reportRequest = hasAny(lowerQuery, ["report", "reports", "reporting", "export", "download", "spreadsheet", "list"]);
  const availabilityRequest = hasAny(lowerQuery, ["availability", "available tee", "tee times", "hot deals", "9 hole", "9-hole", "18 hole", "18-hole", "cannot book", "can't book", "cant book", "visitor availability"]);
  let penalty = 0;
  if (reportRequest && !availabilityRequest && hasAny(titlePath, ["availability", "green fee", "green fees", "hot deals", "course restriction", "member casual booking rules"])) penalty += 14;
  if (!hasAny(lowerQuery, ["competition", "tournament", "open comp", "entry fee", "purse"]) && hasAny(titlePath, ["competition purse", "process competition charges"])) penalty += 12;
  return penalty;
}

function broadEvidencePenalty(query = "", entry = {}) {
  const lowerQuery = normalise(query);
  const titlePath = normalise([entry.title, entry.area, entry.navigationPath, entry.workflowFamily].join(" "));
  let penalty = 0;
  if (titlePath.includes("system tools") && !hasAny(lowerQuery, ["system configuration", "competition purse module", "minimum member competition purse", "tools", "configuration"])) penalty += 20;
  if (titlePath.includes("email and letter templates") && !hasAny(lowerQuery, ["email", "message", "template", "letter"])) penalty += 20;
  if (titlePath.includes("tools") && !hasAny(lowerQuery, ["tools", "brs payments", "transactions", "payouts", "system configuration", "club systems"])) penalty += 6;
  penalty += objectMismatchPenalty(query, entry);
  return penalty;
}
...
function rankEvidence(entries, message, intent) {
  ...
  return entries
    .map((entry) => {
      ...
      const score = base + topicBonus + reportBonus + scoreText(query, allText) + exactPhraseBonus(query, allText) + matchedTokenDensityBonus(query, allText, entry) - broadEvidencePenalty(query, entry) - crossTopicPenalty(intent, entry);
...
async function retrieveLocalEvidenceForAnswer(message, profile, intent, evidencePlan, requiresContextualSynthesis = false) {
  const queries = requiresContextualSynthesis
    ? buildContextualRetrievalQueries(message, profile, intent, evidencePlan)
    : [evidencePlan.queryTerms.join(" ")];
  const batches = await Promise.all(queries.map((query) => retrieveKnowledge(query, { limit: requiresContextualSynthesis ? 12 : 24 }).catch((error) => {
    console.error("Local knowledge retrieval failed:", error);
    return [];
  })));
  return dedupeEvidenceEntries(batches.flat());
}
...
function summarizeEvidenceSource(entry = {}) {
  return {
    title: entry.title || "",
...
function buildEvidencePack({
  ...
  const baseEvidence = compactEvidence(
    ...
    contextProfile?.requiresContextualSynthesis && `QUESTION CONTEXT PROFILE:\n${JSON.stringify(contextProfile, null, 2)}`,
  ...
  const specificsText = formatEvidenceSpecificsPack(extractEvidenceSpecifics(baseEvidence, usefulLocalEntries));
  return {
    sources: usefulLocalEntries.map(summarizeEvidenceSource),
    specificsText,
    baseEvidence,
    text: compactEvidence(
      specificsText && `EXTRACTED WORKFLOW SPECIFICS TO USE IF RELEVANT:\n${specificsText}`,
      baseEvidence,
    ),
  };
}
...
- The answer keeps confidence internal and gives concrete next checks or escalation details when needed.
...
export async function answerFromKnowledgeDetailed(message, options = {}) {
  ...
  let staticWorkflowReply = "";
  let staticIntentEvaluation = null;
  const traceResult = (result, trace = {}) => {
    if (!result) return result;
    result.developerTrace = buildDeveloperAnswerTrace({
      message,
      route: result.route || trace.route || null,
      contextProfile,
      seedIntent,
      intent: trace.intent || result.intent || seedIntent,
      evidencePlan: trace.evidencePlan || evidencePlan,
      staticWorkflowReply,
      staticIntentEvaluation,
      routingEvidence,
      usefulLocalEntries: trace.usefulLocalEntries || [],
      answerComposition: result.answerComposition,
      evidencePack: trace.evidencePack || null,
      fallbackReason: trace.fallbackReason || null,
      qualityIssue: trace.qualityIssue || null,
  ...
  if (!requiresContextualSynthesis && isMoveBookingQuestion(message)) {
    ...
    return traceResult(knowledgeResult(reply, buildAnswerComposition({ mode: "locked-static", evidencePlan, staticReply: reply }), { route: "locked-move-booking" }), { fallbackReason: "locked-move-booking" });
  }
  ...
  if (!requiresContextualSynthesis && isMemberBalanceReportQuestion(message)) {
    ...
    return traceResult(knowledgeResult(reply, buildAnswerComposition({ mode: "locked-static", evidencePlan, staticReply: reply }), { route: "locked-member-balance-report" }), { fallbackReason: "locked-member-balance-report" });
  }

  staticWorkflowReply = approvedStaticWorkflowReply(message);
  staticIntentEvaluation = evaluateStaticAnswerAgainstIntent(message, staticWorkflowReply);
  ...
  if (!requiresContextualSynthesis && isLockedStaticReply(message, staticWorkflowReply)) {
    return traceResult(knowledgeResult(staticWorkflowReply, buildAnswerComposition({ mode: "locked-static", evidencePlan, staticReply: staticWorkflowReply }), { route: "locked-static-safety" }), { fallbackReason: "locked-static-safety" });
  }
  ...
  let contextualFallbackEntries = null;
  const getContextualFallbackEntries = async (intentForFallback = seedIntent, planForFallback = evidencePlan) => {
    if (contextualFallbackEntries) return contextualFallbackEntries;
    const [rawEntries, supportEntries] = await Promise.all([
      retrieveLocalEvidenceForAnswer(message, contextProfile, intentForFallback, planForFallback, true),
      readApprovedSupportFiles(),
    ]);
    contextualFallbackEntries = rankEvidence(
      dedupeEvidenceEntries([...rawEntries.filter((entry) => entry.confidence === "approved"), ...supportEntries]),
      message,
      intentForFallback,
    ).slice(0, 8);
    return contextualFallbackEntries;
  };
  const contextualFallback = async (intentForFallback = seedIntent, planForFallback = evidencePlan, usefulEntries = null, trace = {}) => {
    const fallbackEntries = usefulEntries
      ? rankEvidence(dedupeEvidenceEntries([...usefulEntries, ...await readApprovedSupportFiles()]), message, intentForFallback).slice(0, 8)
      : await getContextualFallbackEntries(intentForFallback, planForFallback);
    const result = knowledgeResult(
      buildContextualSupportFallbackReply({ message, profile: contextProfile, evidenceEntries: fallbackEntries }),
      buildAnswerComposition({ mode: "contextual-evidence-fallback", evidencePlan: planForFallback, staticReply: staticWorkflowReply, usefulLocalEntries: fallbackEntries }),
      { intent: intentForFallback, route: "contextual-evidence-fallback", contextProfile }
    );
    return traceResult(result, {
      intent: intentForFallback,
      evidencePlan: planForFallback,
      usefulLocalEntries: fallbackEntries,
      fallbackReason: trace.fallbackReason || "contextual-synthesis-without-verified-dynamic-answer",
      qualityIssue: trace.qualityIssue || null,
      verification: trace.verification || null,
    });
  };
  ...
  if (!allowDynamic) {
    ...
    return fallbackReply
      ? traceResult(knowledgeResult(fallbackReply, buildAnswerComposition({ mode: "static-fallback", evidencePlan, staticReply: fallbackReply }), { route: "static-fallback-dynamic-disabled" }), { fallbackReason: "dynamic-disabled" })
      : null;
  }
  ...
  if (!shouldAttemptKnowledgeAnswer(message)) {
    if (requiresContextualSynthesis) return contextualFallback();
    return fallbackReply
      ? traceResult(knowledgeResult(fallbackReply, buildAnswerComposition({ mode: "static-fallback", evidencePlan, staticReply: fallbackReply }), { route: "static-fallback-non-attemptable" }), { fallbackReason: "non-attemptable-message" })
      : null;
  }
  ...
  if (!client) {
    if (requiresContextualSynthesis) return contextualFallback();
    return fallbackReply
      ? traceResult(knowledgeResult(fallbackReply, buildAnswerComposition({ mode: "static-fallback", evidencePlan, staticReply: fallbackReply }), { route: "static-fallback-no-openai" }), { fallbackReason: "no-openai-client" })
      : null;
  }
  ...
  if (intent.needsClarification && Number(intent.confidence || 0) < 0.65) {
    ...
    return traceResult(knowledgeResult(reply, buildAnswerComposition({ mode: "clarification", evidencePlan, staticReply: "" }), { intent, route: "intent-clarification" }), { intent, fallbackReason: "low-confidence-intent-clarification" });
    }
  }

  const [localEntriesRaw, helpCenter, approvedSupportContext, decisionTreeContext] = await Promise.all([
    retrieveLocalEvidenceForAnswer(message, contextProfile, intent, evidencePlan, requiresContextualSynthesis),
  ...
  const evidencePack = buildEvidencePack({
    intent,
    evidencePlan,
    contextProfile: requiresContextualSynthesis ? contextProfile : null,
    staticWorkflowReply,
    routingEvidence,
    usefulLocalEntries,
    approvedSupportContext,
  ...
  const baseEvidence = evidencePack.baseEvidence;

  if (!baseEvidence) {
    if (requiresContextualSynthesis) return contextualFallback(intent, evidencePlan, usefulLocalEntries);
    return fallbackReply
      ? traceResult(knowledgeResult(fallbackReply, buildAnswerComposition({ mode: "static-fallback", evidencePlan, staticReply: fallbackReply }), { intent, route: "static-fallback-no-evidence" }), { intent, evidencePlan, fallbackReason: "no-evidence" })
      : null;
  }

  const evidence = evidencePack.text;
...
- For context-heavy customer cases, do not return a generic workflow title and steps. Explain what to check, where to check it, what not to assume, and what to escalate with if live club data is needed.
...
- Avoid final-cause wording when live club data is needed. Do not say "the cause is" or "this is likely" unless the supplied evidence proves it. Keep confidence internal; do not show "Confidence: high/medium/low" in normal answers. If evidence is weak, say plainly that the checks are the safest available route and escalation is needed if they do not explain the record.
...
- For report lookup questions, do not escalate just because there is no single perfect report. If evidence names close report types, recommend the closest report combination and say what each one is used to compare.
- For billing, payment, competition, contact, membership, and report answers, include the specific fields, filters, statuses, reports, and controls the staff member should use when those labels are present in evidence.
...
- If the evidence is not enough, reply exactly: ${UNKNOWN_REPLY}.`,
  ...
  if (!response) {
    if (requiresContextualSynthesis) return contextualFallback(intent, evidencePlan, usefulLocalEntries, { fallbackReason: "generation-timeout" });
    return fallbackReply
      ? traceResult(knowledgeResult(fallbackReply, buildAnswerComposition({ mode: "static-fallback", evidencePlan, staticReply: fallbackReply, usefulLocalEntries, approvedSupportContext, helpCenter, decisionTreeContext }), { intent, route: "static-fallback-generation-timeout" }), { intent, evidencePlan, usefulLocalEntries, evidencePack, fallbackReason: "generation-timeout" })
      : null;
  }

  const reply = response.output_text?.trim();
  if (!reply || reply === UNKNOWN_REPLY) {
    if (requiresContextualSynthesis) return contextualFallback(intent, evidencePlan, usefulLocalEntries, { fallbackReason: "model-returned-unknown" });
    return fallbackReply
      ? traceResult(knowledgeResult(fallbackReply, buildAnswerComposition({ mode: "static-fallback", evidencePlan, staticReply: fallbackReply, usefulLocalEntries, approvedSupportContext, helpCenter, decisionTreeContext }), { intent, route: "static-fallback-unknown" }), { intent, evidencePlan, usefulLocalEntries, evidencePack, fallbackReason: "model-returned-unknown" })
      : null;
  }
  ...
  if (!supported || (!requiresContextualSynthesis && isBRSWorkflowQuestion(message) && !hasWorkflowShape(reply))) {
    const qualityIssue = requiresContextualSynthesis ? contextualAnswerIssue(message, reply) : null;
    if (requiresContextualSynthesis) return contextualFallback(intent, evidencePlan, usefulLocalEntries, { fallbackReason: "verification-failed", qualityIssue, verification: { supported } });
    return fallbackReply
      ? traceResult(knowledgeResult(fallbackReply, buildAnswerComposition({ mode: "static-fallback", evidencePlan, staticReply: fallbackReply, usefulLocalEntries, approvedSupportContext, helpCenter, decisionTreeContext }), { intent, route: supported ? "static-fallback-workflow-shape" : "static-fallback-verification-failed" }), { intent, evidencePlan, usefulLocalEntries, evidencePack, fallbackReason: supported ? "missing-workflow-shape" : "verification-failed", qualityIssue, verification: { supported } })
      : null;
  }
  ...
  return traceResult(
    knowledgeResult(finalReply, dynamicComposition, { intent, route: requiresContextualSynthesis ? "contextual-dynamic-synthesis" : "dynamic-synthesis", contextProfile }),
    { intent, evidencePlan, usefulLocalEntries, evidencePack, verification: { supported: true } }
  );
}
