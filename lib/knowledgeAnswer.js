...
function rankEvidence(entries, message, intent) {
  ...
  return entries
    .map((entry) => {
      ...
      const score = base + topicBonus + reportBonus + scoreText(query, allText) + exactPhraseBonus(query, allText) - broadEvidencePenalty(query, entry) - crossTopicPenalty(intent, entry);
...
export async function answerFromKnowledgeDetailed(message, options = {}) {
  ...
  const contextualFallback = async (intentForFallback = seedIntent, planForFallback = evidencePlan, usefulEntries = null) => {
