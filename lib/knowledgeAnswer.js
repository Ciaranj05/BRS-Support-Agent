...
function isLockedStaticReply(message = "", reply = "") {
  ...
  if (/Configure the Timesheet/i.test(reply) && hasAny(lower, ["tee slot", "tee slots", "tee time", "tee times", "interval", "intervals", "opening earlier", "opening time"])) return true;
  if (/Title for Each Day/i.test(reply) && hasAny(lower, ["label", "title", "top of", "particular day", "timesheet", "tee sheet"])) return true;
  if (/Copy Services, Catering, or Green Fees/i.test(reply) && hasAny(lower, ["copy", "next year", "from year", "to year"])) return true;
  if (/Set Up Legal Messages/i.test(reply) && hasAny(lower, ["privacy policy", "member terms", "terms message", "legal messages", "appears online"])) return true;
  if (/Golf Events vs Competitions/i.test(reply)) return true;
  if (/Set Up an Open Competition for Visitors/i.test(reply)) return true;
  if (/Add a Member to a Competition Waiting List/i.test(reply)) return true;
  if (/Check a Member Bill Payment Against BRS Payments/i.test(reply)) return true;
  if (/Services in BRS are bookable extras/i.test(reply)) return true;
  if (/Email Membership Groups and Text Message Membership Groups/i.test(reply)) return true;
...
export async function answerFromKnowledgeDetailed(message, options = {}) {
  ...
  if (isMoveBookingQuestion(message)) {
  ...
  if (isMemberBalanceReportQuestion(message)) {
  ...
  if (isLockedStaticReply(message, staticWorkflowReply)) {
  ...
  const fallbackReply = staticWorkflowReply || contactReply || timesheetSetupReply || membershipReportReply || deterministicRefund;
...
- If the evidence is not enough, reply exactly: ${UNKNOWN_REPLY}.`,
  ...
  const supported = await verifyReply(message, intent, evidence, reply);
  if (!supported || (isBRSWorkflowQuestion(message) && !hasWorkflowShape(reply))) {
    return fallbackReply
  ...
  return knowledgeResult(finalReply, dynamicComposition, { intent, route: "dynamic-synthesis" });
}
