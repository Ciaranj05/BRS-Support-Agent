...
function isAllMembersUnpaidBillsQuestion(message = "") {
  const lower = normalise(message);
  return hasAny(lower, ["unpaid bill", "unpaid bills", "outstanding bill", "outstanding bills", "owed bills", "owed money", "owe money", "owing money", "overdue bill", "overdue bills", "outstanding balance", "outstanding balances", "arrears", "still owe"])
    && hasAny(lower, ["all members", "every member", "members", "member list", "list", "report", "see which", "show", "view", "which", "who", "download", "export"]);
}
...
2. Go to Billing/Payments to review member bills and payment status.
3. Use the Dashboard view to filter for unpaid or failed payments if available.
4. Check Create Bills and Billing Overview as well, because those areas can show bill status and outstanding balances.
5. If you need the full list, use the available export/download option from the billing/report view.

Filter/check
- Filter by unpaid, outstanding, failed, or unpaid balance status where available.
- Narrow by billing cycle, date range, or payment period if the club only needs a specific renewal window.
- Check member name, bill/invoice status, amount/balance due, due date, and payment status.

For one individual member, open the member profile and check their Billing tab instead of using the all-member list.`;
}
...
For example, if the club wants weekdays and weekends set differently, configure each day group separately.`;
}
...
async function verifyReply(message, intent, evidence, reply) {
  ...
  try {
    const response = await client.responses.create({
      model: "gpt-4.1-mini",
      input: [
        { role: "system", content: "Reply exactly SUPPORTED or UNSUPPORTED. Unsupported means the answer uses unrelated evidence, confuses membership bills with competition purse, confuses membership bill refunds with booking refunds, or invents BRS UI paths/actions not in evidence. Supported answers may include safe generic check/export guidance when evidence says a billing/report/dashboard view exists." },
...
export async function answerFromKnowledge(message) {
  ...
  const response = await client.responses.create({
    ...
    input: [
      {
        role: "system",
        content: `You are a technical support agent for BRS Golf admin/staff users. Use dynamic, evidence-led routing: answer from the supplied evidence that best matches the classified topic/object, not from generic keyword overlap.

Answer style:
- Give a complete first answer. Do not end with "would you like me to..." when safe next steps can be given now.
- Use a readable structure: short title, numbered steps, then "Filter/check" and "Export/download" sections when relevant.
...
- First cross-check the ALL LOCAL SUPPORT SECTIONS list before deciding which detailed evidence to use. If another BRS area could affect the answer, mention that related area briefly rather than ignoring it.
- If exact field labels are not proven, say "filter by unpaid/outstanding status if available" rather than inventing a label.
...
- Do not invent prices, club policies, live club data, member names, or balances.
