...
function sanitizeLookupError(error) {
  ...
  if (/libnss3\.so/i.test(message)) {
    return "Chromium could not start in this serverless runtime because the native library libnss3.so is missing. Configure BRS_LIVE_BROWSER_WS_ENDPOINT to use a managed browser service, or run the bot in a Docker/VM environment with Chrome dependencies installed.";
  }
  if (/Executable doesn't exist|playwright install/i.test(message)) {
    return "Chromium is not installed in this runtime. Redeploy with browser dependencies installed or configure BRS_LIVE_BROWSER_WS_ENDPOINT to use a managed browser service.";
  }
