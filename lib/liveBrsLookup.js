...
async function collectPageEvidence(page) {
  ...
  return {
    ...
    controls: evidence.controls
      .map((control) => ({ ...control, label: redactSensitive(control.label), options: unique((control.options || []).map(redactSensitive), 60) }))
      .filter((control) => control.label && !BLOCKED_ACTION_PATTERN.test(control.label))
      .slice(0, 120),
  };
}
...
export function shouldAttemptLiveBrsLookup(question = "", staticEvidence = "") {
  ...
  const asksWorkflow = /\b(how|where|which|what|show|see|find|list|report|filter|export|download)\b/.test(lower);
  const productArea = /\b(member|membership|bill|payment|refund|booking|tee|competition|report|user|admin|setting|configuration)\b/.test(lower);
...
export function formatLiveEvidence(result = {}) {
  ...
  return result.pages.map((page, index) => [
    ...
    page.controls?.length ? `Controls: ${page.controls.slice(0, 40).map((control) => `${control.label}${control.options?.length ? ` (${control.options.join(" / ")})` : ""}`).join(" | ")}` : null,
  ].filter(Boolean).join("\n")).join("\n\n---\n\n");
}
